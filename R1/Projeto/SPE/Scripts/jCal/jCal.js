...
(function ($) {
	...
	$.jCal = function (target, opt) {
		opt = $.extend({
			...
			customDay: function (day) { return false; },           //CUSTOMIZA O DIA SOMENTE SE NAO TIVER
			customStyle: function (day) { return false; },
		...
		$(target).append(
			'<div class="jCal">' +
					...
					'<div class="month">' +
                        ...
                        '<span class="monthNumber" hidden="hidden">' + opt.day.getMonth() + '</span>' +
					'</div>' +
	...
	function drawCal(target, opt) {
		...
		for (var d = offsetDayStart, dE = (copt.fd + copt.ld + (7 - offsetDayEnd)) ; d < dE; d++) {
			var thisDay = new Date((new Date(fd.getTime())).setDate(d - (copt.fd - opt.dayOffset)));
			var customDay = opt.customDay(thisDay);
			var customStyle = opt.customStyle(thisDay) ? "style='" + opt.customStyle(thisDay) + "'" : "";

			$(target).append(
				((d <= (copt.fd - opt.dayOffset)) ?
					'<div id="' + opt.cID + 'd' + d + '" class="pday" ' + customStyle + ' >' + (customDay ? customDay : (copt.lld - ((copt.fd - opt.dayOffset) - d))) + '</div>'
					: ((d > ((copt.fd - opt.dayOffset) + copt.ld)) ?
						'<div id="' + opt.cID + 'd' + d + '" class="aday" ' + customStyle + '>' + (customDay ? customDay : (d - ((copt.fd - opt.dayOffset) + copt.ld))) + '</div>'
						: '<div id="' + opt.cID + 'd_' + (fd.getMonth() + 1) + '_' + (d - (copt.fd - opt.dayOffset)) + '_' + fd.getFullYear() + '" class="' +
							...
							'" ' + customStyle + ' >' + (customDay ? customDay : (d - (copt.fd - opt.dayOffset))) + '</div>'
					)
				)
			);
		}
		$(target).find('div[id^=' + opt.cID + 'd]:first').before('<br style="clear:both;" />');
		//$(target).find('div[id^=' + opt.cID + 'd]:nth-child(7n+2)').before('<br style="clear:both;" />');
